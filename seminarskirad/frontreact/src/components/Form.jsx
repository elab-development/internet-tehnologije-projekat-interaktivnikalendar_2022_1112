...
const Form = ({ date, onClose, isAdminProp, locations }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const formattedDateForApi = `${date.getFullYear()}-${
        date.getMonth() + 1
      }-${date.getDate()}`;
      const newEvent = {
        naziv: eventTitle,
      ...
      if (
        response.data &&
        response.data[0] &&
        response.data[0].includes("Uspešno kreiran novi dogadjaj")
      ) {
      ...
      alert(
        "Došlo je do greške prilikom komunikacije sa serverom. Administrator ne može da dodaje nove događaje, samo ulogovani korisnik može!"
      );
    }
  };
