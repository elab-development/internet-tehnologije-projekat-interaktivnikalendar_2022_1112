  ...
  const handleAddLocation = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/api/lokacije",
        novaLokacija,
        {
          headers: {
      ...
      alert(
        "Greška prilikom komunikacije sa serverom. Samo admin moze da doda, izmeni i briše lokacije."
      );
    }
  };
  ...
  const handleSacuvajIzmene = async () => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:8000/api/lokacije/${izmenaLokacije.id}`,
        izmenaLokacije,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      <div class="add-location-container">
        ...
        <form onSubmit={handleAddLocation}>
          ...
          <input
            type="text"
            name="poštanski_kod"
            value={novaLokacija.poštanski_kod}
            onChange={handleInputChange}
            ...
            {izmenaLokacije && izmenaLokacije.id === lokacija.id ? (
              <div className="edit-location-form">
                ...
                <input
                  type="text"
                  name="poštanski_kod"
                  value={izmenaLokacije.poštanski_kod}
                  onChange={handleInputChange}
                  placeholder="Poštanski kod"
                />
                <div>
                  <button onClick={handleSacuvajIzmene}>Sačuvaj izmene</button>
                  <button onClick={() => setIzmenaLokacije(null)}>
                    Odustani
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <button onClick={() => handleIzbrisiLokaciju(lokacija.id)}>
                  Izbriši
                </button>
              </div>
            )}
